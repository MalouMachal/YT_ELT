...
def insert_rows(cur, conn, schema, row):
    ...
    try:
        ...
        if schema == "staging":
            ...
            cur.execute(  #here the schema would be the staging scehma and the table yt_api
                f"""  
                ...
                """,
                row,  # is the arg 2 of the execute commande, that contains a specific row
            )

        else:   #for the core_scehma
        ...
        conn.commit()   #we commit our changes as we are doing some changes to our database
...
def delete_rows(cur, conn, schema, ids_to_delete):
    ...
    try:
            #we have to use the join method to change the format of the string
